...
RIni_Read(RVar, File, Correct_Errors=1, Comments_Under_Key_For_Key=0, Remove_Inline_Key_Comments=0, Remove_Lone_Line_Comments=0, Remove_Inline_Section_Comments=0, Treat_Duplicate_Sections=1, Treat_Duplicate_Keys=1, Read_Blank_Sections=1, Read_Blank_Keys=1, Trim_Spaces_From_Values=0, ByRef RIni_Read_Var = "")
{
	...
	;3 - Replace
	Global
	Local Has_Equal, Sec, Key, PrevKey, P_1, P_2, Section_Skip, C_Pos, Section_Append, T_Sections, T_Section, E, T_LoopField, Errored_Lines, T_Value
		...
		If (SubStr(T_Loopfield, 1, 1) = ";"){
			If !Section_Skip
			{
				If !Remove_Lone_Line_Comments
				{
					...
					{
						; --- MODIFICATION START (dein0s) ---
						; this mod allows to treat comments under key=value pair as comments for the key
						; otherwise they will be treated as lone line comments
						PrevKey := (%RVar%_%Sec%_PrevKey) ? RIni_CalcMD5(%RVar%_%Sec%_PrevKey) : 0
						If (PrevKey and Comments_Under_Key_For_Key)
							If %RVar%_%Sec%_%PrevKey%_Comment
								%RVar%_%Sec%_%PrevKey%_Comment .= "`n" A_LoopField
							Else
								%RVar%_%Sec%_%PrevKey%_Comment .= A_LoopField
						Else
...
RIni_Write(RVar, File, Newline="`r`n", Write_Blank_Sections=1, Write_Blank_Keys=1, Write_Key_Comments_Inline=1, Space_Sections=1, Space_Keys=0, Remove_Valuewlines=1, Overwrite_If_Exists=1, Addwline_At_End=0)
{
	Global
			...
			If (%RVar%_All_%T_Section%_Keys){
				Loop, Parse, %RVar%_All_%T_Section%_Keys, `n
				{
					...
					If %RVar%_%T_Section%_%A_LoopField%_Comment {
						; --- MODIFICATION START (dein0s) ---
						T_Comment := %RVar%_%T_Section%_%A_LoopField%_Comment
						If (Write_Key_Comments_Inline) {
							If InStr(T_Comment, "`n")
								StringReplace, T_Comment, T_Comment, `n, %A_Space%, A
							If InStr(T_Comment, "`r")
								StringReplace, T_Comment, T_Comment, `r, %A_Space%, A
							Write_Ini .= T_Key "=" T_Value T_Comment Newline
						}
						Else
							Write_Ini .= T_Key "=" T_Value Newline T_Comment Newline
						; --- MODIFICATION END (dein0s) ---
					}
					Else {
	...
	If (InStr(Comment, "`n") or InStr(Comment, "`r")){
		Loop, parse, Comment, `n, `r
		{
			If A_LoopField =
				Continue
			If %RVar%_%Sec%_%Key%_Comment
				%RVar%_%Sec%_%Key%_Comment .= A_Space A_LoopField
			Else
				%RVar%_%Sec%_%Key%_Comment .= ";" A_LoopField
		}
	} Else {
		If %RVar%_%Sec%_%Key%_Comment
			%RVar%_%Sec%_%Key%_Comment .= A_Space Comment
		Else
